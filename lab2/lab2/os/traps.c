	     ...
	     uint32 *trapArgs)
{
  ...
  if (cause & TRAP_TRAP_INSTR) {
    ...
    case TRAP_SEM_WAIT:
      ihandle = GetIntFromTrapArg(trapArgs, isr & DLX_STATUS_SYSMODE);
      handle = SemHandleWait(ihandle); //calls SemHandleWait, which calls SemWait, which decreases the semaphore value and if it goes below zero, it will sleep and wait for one to be signaled
      ProcessSetResult(currentPCB, handle); //Return SYNC_SUCCESS or SYNC_FAIL
      break;
    case TRAP_SEM_SIGNAL:
      ihandle = GetIntFromTrapArg(trapArgs, isr & DLX_STATUS_SYSMODE); //get the semaphore handle from the function call
      handle = SemHandleSignal(ihandle); //calls SemHandleSignal, which calls SemSignal, which increments the semaphore and wakes a sleeping process if any are waiting
      ProcessSetResult(currentPCB, handle); //Return SYNC_SUCCESS or SYNC_FAIL
      break;
