...
void main (int argc, char *argv[])
{
  ...
  mbox_handle_NO = dstrtol(argv[1], NULL, 10); // The "10" means base 10
  ...
  if(mbox_open(mbox_handle_NO) == MBOX_FAIL) {
  ...
  if (mbox_recv(mbox_handle_NO, 10, (void *)recv_buf) == MBOX_FAIL) {
  ...
  if(mbox_send(mbox_handle_N2, sizeof(content_n2), (void *)content_n2) == MBOX_FAIL) {
      Printf("Could not send message to mailbox %d in %s (%d)\n", mbox_handle_N2, argv[0], getpid());
      Exit();
  }
  ...
  if (mbox_close(mbox_handle_NO) == MBOX_FAIL) {
    Printf("makeprocs (%d): Could not open mailbox %d!\n", getpid(), mbox_handle_NO);
    Exit();
  }
