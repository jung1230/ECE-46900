...
void ProcessSchedule () { //LOTTERY
    ...
    int was_waiting;
    int prev_total;
    int something_sleeping;
    int upper;

    //if nothing running and nothing waiting, quit
    if(AQueueEmpty(&waitQueue) && AQueueEmpty(&runQueue)){
        exitsim();
    }

    //if the idle process is the only thing running and nothing is waiting, exit
    if ((AQueueLength(&runQueue) == 1) && AQueueEmpty(&waitQueue)) {
      l = AQueueFirst(&runQueue);
      pcb = (PCB *)AQueueObject(l);
      if(pcb == globalIdlePCB){
        exitsim();
      }
    }

    //update the total time
    currentPCB->total_time += (ClkGetCurJiffies() - currentPCB->prev_sched_time);

    //if printing is enabled, print
    if(currentPCB->pinfo == 1){
      printf(PROCESS_CPUSTATS_FORMAT, GetPidFromAddress(currentPCB), currentPCB->total_time, currentPCB->pnice);
    }

    something_sleeping = 0;

    //go through and wake up everygtiung that needs to be woken up
    if (!AQueueEmpty(&waitQueue)) {
      ...
      while (l != NULL) {
        ...
        //if we found something thats sleeping
        if(pcb->sleeping == 1){

          //if its been asleep long enough, wake it
          if(pcb->time_to_sleep <= (ClkGetCurJiffies() - pcb->time_went_sleep)){
            pcb->sleeping = 0;
            ProcessWakeup (pcb);
          }
        }
      ...
      //go through again, and check if anything is still sleeping
      l = AQueueFirst(&waitQueue);
      while (l != NULL) {
        pcb = (PCB *)AQueueObject(l);
        //if we found something thats sleeping, mark it
        if(pcb->sleeping == 1){
          something_sleeping = 1;
          break;
        }
        l = AQueueNext(l);
      }

      //something sleeping, nothing running, so go idle
      if((something_sleeping == 1) && (AQueueEmpty(&runQueue))){ 
        ProcessFork (ProcessIdle, 0, 1, 0, NULL, 0);
        currentPCB = globalIdlePCB;
        return;
      }
    }

    //check if the current pcb is stil running
    was_running = 0;
    if (currentPCB->flags & PROCESS_STATUS_RUNNABLE){
      was_running = 1;
    }

    //check if the current pcb is on wait queue
    was_waiting = 0;
    if (!AQueueEmpty(&waitQueue)) {
      l = AQueueFirst(&waitQueue);
      while (l != NULL) {
        pcb = (PCB *)AQueueObject(l);
        if(GetPidFromAddress(pcb) == GetPidFromAddress(currentPCB)){
          was_waiting = 1;
        }
        l = AQueueNext(l);
      }
    ...
    if((currentPCB->sleeping == 0) && (currentPCB->yield == 0)){ //not sleepign not yielding
      
      //if the current pcb is waiting, increase tickets
      if(was_running == 0){
        
        //increase tickets of process that just finished
        currentPCB->pnice++;

      }
      else if(currentPCB->total_time > (ClkGetCurJiffies() - currentPCB->prev_sched_time)){ //if the current pcb is running, decrease tickets
        
        //decrease tickets that just finished
        currentPCB->pnice--;
      
      }
    }
    else{ //was yielding or sleeping

      //if it was yielding, reset the yield flag and move to end of run queue
  ...
  //check to make sure tickets are between 1 and 19
  ...
  //initialize tickets to zero before we total them
  total_tickets = 0;

  //if the wait queue has stuff on it, loop through them all and total up the number of tickets
  if (!AQueueEmpty(&runQueue)) {

    //take the first link off the waiting queue
    l = AQueueFirst(&runQueue);

    //add this process tickets to total
    while (l != NULL) {
      pcb = (PCB *)AQueueObject(l);

      if ((currentPCB->yield == 0) && (currentPCB->sleeping == 0)){
  ...
  //loop through again and select process to run
  if (!AQueueEmpty(&runQueue)) {

    //take the first link off the running queue
    l = AQueueFirst(&runQueue);

    //while we have a thing from the queue
    while (l != NULL) {

      //pull out the pcb
      pcb = (PCB *)AQueueObject(l);

      //if random num is in this process's range, schedule it for next to run
      if((prev_total <= random_num) && (random_num <= (prev_total + pcb->pnice))){
      ...
      //increase range of tickets
      prev_total += pcb->pnice;
  ...
  //save the time that this next quantum started
  currentPCB->prev_sched_time = ClkGetCurJiffies();
}
#endif
...
void ProcessSchedule () {
  ...
  //only get here if there is something in run queue --------
...
int ProcessFork (VoidFunc func, uint32 param, int pnice, int pinfo,char *name, int isUser) {
  ...
  stackframe = ((uint32 *)(pcb->sysStackArea + MEMORY_PAGE_SIZE)) - (PROCESS_STACK_FRAME_SIZE + 8);
...
void ProcessUserSleep(int seconds) {
  // Your code here
  currentPCB->time_went_sleep = ClkGetCurJiffies();
  currentPCB->time_to_sleep = seconds / 1000;
