...
void ProcessFreeResources (PCB *pcb) {
  ...
  if(pcb->npages != 0){
    for(i = 0; i < L1_PT_SIZE; i++){
      MemoryFreeL2Table(pcb, pcb->pagetable[i]);
      pcb->pagetable[i] = 0;
    }
  }
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  //---------------------------------------------------------
  //printf("start of process allocation\n");

  codeL2Table = AllocateL2Table(); //allocate a new L2 table for code
  //printf("l2 table allocated for code: %d\n", codeL2Table);
  addNewL2toPcbL1(pcb, codeL2Table, 0); //put the L2 table into the pcb's L1 table for code

  //printf("allocated l2 table num for code: %d\n", codeL2Table);
  //printf("base adr of new table (%d): %x\n", codeL2Table, pcb->pagetable[codeL2Table]);

  //allocate 4 pages for user code and global data
  pcb->npages = 0;
  for(i = 0; i < 4; i++){
    newPage = MemoryAllocPage (); //allocate page
    //printf("allocated page num for code: %d\n", newPage);
    addEntryToL2Table(pcb, MemorySetupPte(newPage), 0, i);
    //printf("pte added to table %d: %x\n\n", codeL2Table, MemorySetupPte(newPage));
  }

  //user
  stackL2Table = AllocateL2Table();
  //printf("l2 table allocated for stack: %d\n", stackL2Table);
  addNewL2toPcbL1(pcb, stackL2Table, 1);
  newPage = MemoryAllocPage (); //allocate user page
  addEntryToL2Table(pcb, MemorySetupPte(newPage), 15, L1_PT_SIZE - 1);
  pcb->npages++; //increment page counter
  //printf("base address of l2 page table for user stack: %x\n", pcb->pagetable[15]);

  //system 
  newPage = MemoryAllocPage ();
  pcb->sysStackArea = newPage * MEM_PAGESIZE;

  //printf("system stack pointer: %x\n", pcb->sysStackArea);
  ...
  stackframe[PROCESS_STACK_PTBITS] = (MEM_L1FIELD_FIRST_BITNUM
					  | (MEM_L2FIELD_FIRST_BITNUM << 16));
  //printf("after setting stackframe\n");
  ...
  if (isUser) {
    ...
    //----------------------------------------------------------------------
    //printf("setting user stack pointer\n");
    stackframe[PROCESS_STACK_USER_STACKPOINTER] = 0xffffc;//= (MAX_PAGES * MEM_PAGESIZE) - 4;
