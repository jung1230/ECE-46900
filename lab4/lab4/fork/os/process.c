...
void ProcessFreeResources (PCB *pcb) {
  ...
  //------------------------------------------------------------
  //printf("process death\n");
  if(pcb->npages != 0){
    for(i = 0; i < L1_PT_SIZE; i++){
      if((pcb->pagetable[i] != 0) && ((pcb->pagetable[i] & MEM_PTE_VALID) != 0)){
        /* reference_counter[pcb->pagetable[i] >> MEM_L1FIELD_FIRST_BITNUM]--;
        if(reference_counter[pcb->pagetable[i] >> MEM_L1FIELD_FIRST_BITNUM] == 0){
          printf("freeing page: %d\n", pcb->pagetable[i] >> MEM_L1FIELD_FIRST_BITNUM);
          MemoryFreePage(pcb->pagetable[i] >> MEM_L1FIELD_FIRST_BITNUM);
        } */
        //printf("reference counter %d\n", pcb->pagetable[i] >> MEM_L1FIELD_FIRST_BITNUM);
        if(reference_counter[pcb->pagetable[i] >> MEM_L1FIELD_FIRST_BITNUM] > 1){
          //printf("decreasing reference counter %d\n", pcb->pagetable[i] >> MEM_L1FIELD_FIRST_BITNUM);
          reference_counter[pcb->pagetable[i] >> MEM_L1FIELD_FIRST_BITNUM]--;
        }
        else{
          reference_counter[pcb->pagetable[i] >> MEM_L1FIELD_FIRST_BITNUM]--;
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  //allocate 4 pages for user code and global data
  pcb->npages = 0;
  for(i = 0; i < 4; i++){
    newPage = MemoryAllocPage ();
    reference_counter[newPage]++;
    //printf("code/global date page #: %d\n", newPage);
    pcb->pagetable[i] = MemorySetupPte (newPage);
    //pcb->npages++;
  }

  //user
  newPage = MemoryAllocPage (); //allocate user page
  //printf("allocated page1: %d\n", newPage);
  reference_counter[newPage]++;
  pcb->pagetable[L1_PT_SIZE - 1] = MemorySetupPte (newPage); //set highest PTE to that page
  //printf("user stack page: %x\n", pcb->pagetable[L1_PT_SIZE - 1]);
  pcb->npages++; //increment page counter

  //system 
  newPage = MemoryAllocPage ();
  //printf("allocated page2: %d\n", newPage);
  //reference_counter[newPage]++;
  pcb->sysStackArea = newPage * MEM_PAGESIZE;

  //move the system stack pointer to the highest address of the system stack page
  stackframe = ((uint32 *)(pcb->sysStackArea + (MEM_PAGESIZE - 1) - 3));
  //printf("highest address in system stack: %x\n", stackframe);
  ...
  stackframe[PROCESS_STACK_PTSIZE] = L1_PT_SIZE;
  stackframe[PROCESS_STACK_PTBITS] = (MEM_L1FIELD_FIRST_BITNUM
					  + (MEM_L1FIELD_FIRST_BITNUM << 16));
  ...
  if (isUser) {
    ...
    //----------------------------------------------------------------------
    stackframe[PROCESS_STACK_USER_STACKPOINTER] = (L1_PT_SIZE * MEM_PAGESIZE) - 4;
...
int ROP_Handler(PCB *pcb){
  ...
  //printf("HERE enter: %d, PID: %d\n", pcb->pagetable[255] >> MEM_L1FIELD_FIRST_BITNUM, GetCurrentPid());

  fault_addr = pcb->currentSavedFrame[PROCESS_STACK_FAULT];
  page = (fault_addr >> MEM_L1FIELD_FIRST_BITNUM);

  //printf("physical page num: %d\n", pcb->pagetable[page] >> MEM_L1FIELD_FIRST_BITNUM);
  
  //if the page is being used by more than one process
  if(reference_counter[pcb->pagetable[page] >> MEM_L1FIELD_FIRST_BITNUM] > 1){
    
    //create new page and copy memory to it
    reference_counter[pcb->pagetable[page] >> MEM_L1FIELD_FIRST_BITNUM]--;
    newPage = MemoryAllocPage();
    //printf("allocated page3: %d\n", newPage);
    //printf("newpage: %d\n", newPage);
    //reference_counter[newPage]++;
    MemoryCopySystemToUser(pcb, (char *) (fault_addr & ~MEM_ADDRESS_OFFSET_MASK) , buffer, 4096);
    MemoryCopyUserToSystem(pcb, buffer, (char *) newPage, 4096);
    //printf("newpage: %x\n", newPage);
    //printf("HERE leave 3: %d, PID: %d\n", pcb->pagetable[255] >> MEM_L1FIELD_FIRST_BITNUM, GetCurrentPid());
    
    //replace current PTE with the new one
    //printf("page: %d\n", page);
    pcb->pagetable[page] = MemorySetupPte(newPage);
    pcb->pagetable[page] &= ~MEM_PTE_READONLY;

    //printf("HERE leave 4: %d, PID: %d\n", pcb->pagetable[255] >> MEM_L1FIELD_FIRST_BITNUM, GetCurrentPid());

    return MEM_SUCCESS;

  }
  else if(reference_counter[pcb->pagetable[page] >> MEM_L1FIELD_FIRST_BITNUM] == 1){
    //mark bit read write
    pcb->pagetable[page] &= ~(MEM_PTE_READONLY);
    return MEM_SUCCESS;
  }

  //printf("HERE leave 2: %d, PID: %d\n", pcb->pagetable[255] >> MEM_L1FIELD_FIRST_BITNUM, GetCurrentPid());
  return MEM_FAIL;
}
...
void ProcessRealFork(){
  ...
  //get a new pcb from the free pcbs
  ...
  for(i = 0; i < L1_PT_SIZE; i++){ 
    ...
    //if the PTE was valid
    if(currentPCB->pagetable[i] & MEM_PTE_VALID){

      //mark as read only
      currentPCB->pagetable[i] |= MEM_PTE_READONLY;

      //incremement reference counter since a new process is going to have access to this page
      reference_counter[currentPCB->pagetable[i] >> MEM_L1FIELD_FIRST_BITNUM]++;
    } 
  } 

  //copy the current pcb into the child pcb
  bcopy((char*)currentPCB, (char*)childPCB, sizeof(PCB));

  //printf("HERE: %d\n", childPCB->pagetable[255] >> MEM_L1FIELD_FIRST_BITNUM);

  //set up system stack
  newPage = MemoryAllocPage();
  //printf("allocated page5: %d\n", newPage);
  //reference_counter[newPage]++;
  childPCB->sysStackArea = newPage * MEM_PAGESIZE; //new page base address

  //copy content the system stack to the child stack
  bcopy((char*)(currentPCB->sysStackArea), (char*)(childPCB->sysStackArea), 4096);

  //fix all the values that need to be fixed (sysStackPtr, currentSavedFrame, and PTBASE)
  ...
  //allocate a queue link for the child pcb
  childPCB->l = AQueueAllocLink(childPCB);
  ...
  //return actual child pid
  ProcessSetResult(currentPCB, (int)(childPCB - pcbs));

  //return 0 for fork call to tell if child
  ProcessSetResult(childPCB, 0); 
}

void PrintVpteHandler(){
  int i;
  uint32 vir_pagenum;
  uint32 phys_pagenum;
  uint32 phys_addr;
  ...
  printf("VPTE start (%d)\n", GetCurrentPid());
  for(i = 0; i < L1_PT_SIZE; i++){
    if((currentPCB->pagetable[i] & MEM_PTE_VALID) != 0){
      vir_pagenum = i; //confirmed
      phys_pagenum = currentPCB->pagetable[i] >> MEM_L1FIELD_FIRST_BITNUM; //confirmed
      phys_addr = currentPCB->pagetable[i] & PTE_TO_ADDRESS;
      printf("Virtual Page Number: %d, Physical Page Number: %d, Physical Addr: 0x%x\n", vir_pagenum, phys_pagenum, phys_addr);
    }
  }
  printf("VPTE end (%d)\n", GetCurrentPid());
}